...
final StateNotifierProvider<FavoritePlaceViewModel, FavoriteViewState> favoriteProvider =
StateNotifierProvider<FavoritePlaceViewModel, FavoriteViewState>(
      (StateNotifierProviderRef<FavoritePlaceViewModel, FavoriteViewState> ref) => FavoritePlaceViewModel(
    placesRepository: injector<PlacesLocalRepository>(),
  ),
);


class FavoritePlaceViewModel extends ViewModelAbs<FavoritePlaceViewModel, FavoriteViewState> {
  ...
  Future<void> getFavoritePlaces() async {
    Result<List<ResultEntity>> result = await _placesRepository
        .getFavoritePlaces();
    if (result is Success) {
      print('oyoyoyoyyoyo');
      state.init((result as Success<List<ResultEntity>>).data);
    } else {
  ...
  Future<void> putFavPlaces(ResultEntity resultEntity) async {
    Result<void> result = await _placesRepository.putFavoritePlaces(
        resultEntity);
    if (result is Success) {
      print('Success');
      return;
